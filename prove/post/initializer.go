...
type Initializer *C.Initializer
...
// NewInitializer calls the C function from libpostrs that creates the
// initializer.
func NewInitializer(providerID, n uint32, commitment, vrfDifficulty []byte) (Initializer, error) {
	cProviderId := C.uint32_t(providerID)
	cN := C.uintptr_t(n)
	cCommitment := C.CBytes(commitment)
	defer C.free(cCommitment)
	cDifficulty := C.CBytes(vrfDifficulty)
...
// FreeInitializer calls the C function from libpostrs that frees the memory
// allocated for the initializer.
func FreeInitializer(init Initializer) {
	C.free_initializer(init)
}

// ScryptPositions calls the C functions from libpostrs that create the labels
// and VRF proofs.
func ScryptPositions(init Initializer, start, end uint64) ([]byte, *uint64, error) {
